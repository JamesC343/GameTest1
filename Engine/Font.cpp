...
void Font::DrawText(const std::string& text, const Vector<int>& pos, Color color, Graphics* gfx) const
{
	...
	for (auto c : text)
	{
		...
		else if (c >= firstChar + 1 && c <= lastChar)
		{
			...
			gfx->DrawSpriteSubstitute(curPos.x, curPos.y, color, MapGlyphRect(c), surface);
		}
